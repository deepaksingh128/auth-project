...
const ResetPassword = () => {
  ...
  const onSubmitEmail = async (e) => {
    ...
    try {
      ...
      data.success ? toast.success(data.messsage) : toast.error(data.messsage);
...
{!isEmailSent && 
      <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='mb-4 flex items-center gap-3 w-ful px-5 py-2.5
        rounded-full bg-[#333A5C]'>
          <img src={assets.mail_icon} alt="" className='w-3 h-3' />

          <input type="email" placeholder='Email id' value={email}
          className='bg-transparent bg-none text-white' required 
...
{!isOtpSubmitted && isEmailSent && 
      <form onSubmit={onSubmitOtp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'> 
        ...
        <p className='text-center mb-6 text-indigo-300'>
          Enter the 6 digit code sent to your id.
        </p>
